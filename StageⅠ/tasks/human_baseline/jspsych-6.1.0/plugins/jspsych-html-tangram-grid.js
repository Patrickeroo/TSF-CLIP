...
jsPsych.plugins["html-tangram-grid"] = (function() {

  var plugin = {};

  plugin.info = {
    name: 'html-tangram-grid',
    description: '',
    parameters: {
      stimulus: {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    html += '<div id="jspsych-html-button-response-btngroup" style="display: grid; grid-template-columns: repeat(5, 1fr);">';
    for (var i = 0; i < trial.choices.length; i++) {
    ...
    function end_trial() {
      ...
      var cleanedResponse = trial.choices[_.toInteger(response.button)]
          .replace('<img width="200px" src="images/', '')
          .replace('.png"></img>', '');
